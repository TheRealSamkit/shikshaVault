<?php

use Illuminate\Support\Facades\Route;
use Illuminate\Support\Facades\Auth;

// Public Page
Route::get('/', function () {
    return view('welcome');
});
Route::get('/resource/{slug}', function () {
    return view('file.view');
})->name('file.view');

Auth::routes(['verify' => true]);

Route::middleware(['auth', 'verified'])->group(function () {
    Route::get('/file/pdf-viewer/{slug}', [App\Http\Controllers\FileAccessController::class, 'viewPdfWrapper'])
        ->name('file.view-pdf');
    Route::get('/home', [App\Http\Controllers\HomeController::class, 'index'])
        ->name('dashboard');

    Route::get('/file/download/{slug}', [App\Http\Controllers\FileAccessController::class, 'download'])->name('file.download');
    Route::get('/file/preview/{slug}', [App\Http\Controllers\FileAccessController::class, 'preview'])->name('file.preview');

    Route::get('/upload', [App\Http\Controllers\FileUploadController::class, 'create'])->name('upload.create');

    Route::get('/my-files', function () {
        return view('uploads');
    })->name('uploads');
    Route::get('/profile', [App\Http\Controllers\ProfileController::class, 'show'])->name('profile.show');
});

Route::middleware(['auth', 'role:admin'])->group(function () {
    Route::get('/admin/dashboard', function () {
        return view('admin.dashboard');
    })->name('admin.dashboard');
    Route::get('/admin/test', function () {
        return view('admin.test');
    })->name('admin.test');

    Route::delete('/file/{id}', [App\Http\Controllers\AdminController::class, 'deleteFile'])
        ->name('admin.file.delete');


    Route::get('/admin/dashboard', [App\Http\Controllers\AdminController::class, 'index'])
        ->name('admin.dashboard');

    Route::get('/admin/curriculum', [App\Livewire\Curriculum::class, 'render'])
        ->name('admin.curriculum');

    Route::post('/user/{id}/toggle', [App\Http\Controllers\AdminController::class, 'toggleUserStatus'])
        ->name('admin.user.toggle');
});

?>